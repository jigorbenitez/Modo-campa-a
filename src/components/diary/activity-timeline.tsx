...
export function ActivityTimeline({
  ...
  return (
    <div className="relative space-y-4 before:absolute before:bottom-4 before:left-[1.95rem] before:top-4 before:hidden before:w-px before:bg-[var(--border)] sm:before:block">
      {records.map((record) => (
        <div id={`activity-${record.activity.id}`} key={record.activity.id} className="relative scroll-mt-24 sm:pl-20">
          ...
          <ActivityCard record={record} defaultOpen={record.activity.id === newestId} onEdit={onEdit} onDelete={onDelete} onDuplicate={onDuplicate} onComplete={onComplete} onExport={onExport} />
        </div>
      ))}
    </div>
  );
