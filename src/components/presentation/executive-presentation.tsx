...
const TerritoryMap = dynamic(
  ...
  {
    ssr: false,
    loading: () => <div className="grid h-full place-items-center bg-[#15231c] text-sm font-bold text-white/45">Preparando territorio…</div>,
  },
);
...
export function ExecutivePresentation() {
  ...
  return (
    <main className="min-h-screen bg-[#09140f] text-white">
      <header className="flex h-20 items-center justify-between border-b border-white/8 px-5 sm:px-8">
        <div className="flex items-center gap-3">
          <span className="grid size-10 place-items-center rounded-xl bg-emerald-400 text-xs font-black text-emerald-950">MC</span>
      ...
      <div className="grid min-h-[calc(100vh-5rem)] xl:grid-cols-[minmax(0,1.45fr)_minmax(22rem,0.55fr)]">
        <section className="relative min-h-[58vh] border-b border-white/8 xl:border-b-0 xl:border-r">
          ...
          <div className="pointer-events-none absolute inset-x-4 top-4 z-[500] grid grid-cols-2 gap-2 sm:grid-cols-4">
            {[
              ...
              <article key={label} className="rounded-2xl border border-white/10 bg-[#0b1711]/88 p-4 shadow-xl backdrop-blur-xl">
          ...
          <div className="absolute inset-x-4 bottom-4 z-[500] rounded-2xl border border-white/10 bg-[#0b1711]/90 p-4 backdrop-blur-xl">
            ...
            <input aria-label="Período de presentación" type="range" min={0} max={mockTerritorySnapshot.periods.length - 1} value={periodIndex} onChange={(event) => setPeriodIndex(Number(event.target.value))} className="mt-3 w-full accent-emerald-400" />
        ...
        <aside className="p-5 sm:p-7">
          <p className="text-xs font-black uppercase tracking-[0.16em] text-emerald-300">Dashboard ejecutivo</p>
